...
import org.nuxeo.ecm.automation.client.jaxrs.spi.marshallers.RawEsMarshaller;
...
public class JsonMarshalling {
	...
	public static class ThrowableDeserializer extends
		...
		public Object deserializeFromObject(JsonParser jp,
			...
			final RemoteThrowable t = (RemoteThrowable) super.deserializeFromObject(jp, ctxt);
	...
	public static JsonFactory newJsonFactory() {
		...
		final ObjectMapper oc = new ObjectMapper(jf);
		...
		oc.getDeserializationConfig().addHandler(new DeserializationProblemHandler() {
			...
			public boolean handleUnknownProperty(
				...
				if (deserializer instanceof ThrowableDeserializer) {
					final JsonParser jp = ctxt.getParser();
					final JsonNode propertyNode = jp.readValueAsTree();
	...
	static {
		...
		addMarshaller(new RawEsMarshaller());
	}
	...
	public static OperationRegistry readRegistry(String content)
			throws Exception {
		final HashMap<String, OperationDocumentation> ops = new HashMap<String, OperationDocumentation>();
		final HashMap<String, OperationDocumentation> chains = new HashMap<String, OperationDocumentation>();
		final HashMap<String, String> paths = new HashMap<String, String>();

		final JsonParser jp = factory.createJsonParser(content);
	...
	private static void readOperations(JsonParser jp,
		...
		while (tok != JsonToken.END_ARRAY) {
			final OperationDocumentation op = JsonOperationMarshaller.read(jp);
			ops.put(op.id, op);
			tok = jp.nextToken();
		}
	}
	...
	public static Object readEntity(String content) throws Exception {
		...
		final JsonMarshaller<?> jm = marshallersByType.get(etype);
		if (jm == null) {
	...
	public static String writeRequest(OperationRequest req) throws Exception {
		...
		final OperationInput input = req.getInput();
		final JsonGenerator jg = factory.createJsonGenerator(writer);
		jg.writeStartObject();
		if (input != null && !input.isBinary()) {
			final String ref = input.getInputRef();
			if (ref != null) {
