...
public class DefaultCMSCustomizer implements INuxeoCustomizer {
	...
	/** The plugin mgr. */
	private final CustomizationPluginMgr pluginMgr;

	/** Avatar map. */
	private final Map<String, String> avatarMap = new ConcurrentHashMap<String, String>();

	/** binary map. */
	private final Map<String, String> binaryMap = new ConcurrentHashMap<String, String>();
	...
	public DefaultCMSCustomizer(PortletContext ctx) {
		...
		// Bundle factory
		final IInternationalizationService internationalizationService = (IInternationalizationService) this.portletCtx.getAttribute(Constants.INTERNATIONALIZATION_SERVICE_NAME);
	...
	public List<ListTemplate> getListTemplates(Locale locale) {
		final List<ListTemplate> templates = new ArrayList<ListTemplate>();

		// Bundle
		final Bundle bundle = this.bundleFactory.getBundle(locale);
		...
		// Slider
		final ListTemplate slider = new ListTemplate(LIST_TEMPLATE_SLIDER, bundle.getString("LIST_TEMPLATE_SLIDER"), SLIDER_SCHEMAS);
	...
	public List<FragmentType> getFragmentTypes(Locale locale) {
		final List<FragmentType> fragmentTypes = new ArrayList<FragmentType>();
		...
		fragmentTypes.add(new FragmentType(NavigationPictureFragmentModule.ID, bundle.getString("FRAGMENT_TYPE_NAVIGATION_PICTURE"), NavigationPictureFragmentModule.getInstance()));
		// Document attachment picture fragment
		fragmentTypes.add(new FragmentType(DocumentPictureFragmentModule.ID, bundle.getString("FRAGMENT_TYPE_DOCUMENT_PICTURE"), DocumentPictureFragmentModule.getInstance()));
		// Link fragment
		...
		fragmentTypes.add(new FragmentType(SitePictureFragmentModule.ID, bundle.getString("FRAGMENT_TYPE_SITE_PICTURE"), SitePictureFragmentModule.getInstance()));
		// Summary fragment
	...
	public SortedMap<String, String> getMenuTemplates(Locale locale) {
		final SortedMap<String, String> templates = new TreeMap<String, String>();
	...
	public CMSHandlerProperties getCMSDefaultPlayer(CMSServiceCtx ctx) throws Exception {
		final Document doc = (Document) ctx.getDoc();
	...
	protected String createFolderRequest(CMSServiceCtx ctx, boolean ordered) throws CMSException {
		...
		final CMSPublicationInfos pubInfos = this.cmsService.getPublicationInfos(ctx, doc.getPath());
		...
		if (ctx.getContextualizationBasePath() != null) {
			...
			navItems = this.cmsService.getPortalNavigationSubitems(ctx, ctx.getContextualizationBasePath(), DocumentPublishSpaceNavigationCommand.computeNavPath(doc.getPath()));
		}
	...
	public CMSHandlerProperties getCMSAnnonceFolderPlayer(CMSServiceCtx ctx) throws CMSException {
		...
		final CMSHandlerProperties linkProps = new CMSHandlerProperties();
	...
	public CMSHandlerProperties getCMSPlayer(CMSServiceCtx ctx) throws Exception {
		...
		if (("Folder".equals(doc.getType()) || "OrderedFolder".equals(doc.getType())) || ("Section".equals(doc.getType()))) {
			...
			if ((ctx.getContextualizationBasePath() != null) && !doc.getTitle().startsWith("test-list")) {
				...
				final CMSItem spaceConfig = this.cmsService.getSpaceConfig(ctx, ctx.getContextualizationBasePath());
				...
				final String spaceType = ((Document) spaceConfig.getNativeItem()).getType();
				if ("Workspace".equals(spaceType) || "UserWorkspace".equals(spaceType)) {
					...
					// Pas de filtre sur les versions publiées
					ctx.setDisplayLiveVersion("1");
					final CMSHandlerProperties props = this.getCMSFileBrowser(ctx);
		...
		// compute domain path
		final String domainPath = WebConfigurationHelper.getDomainPath(ctx);

		if (domainPath != null) {
			// get configs installed in nuxeo
			final WebConfigurationQueryCommand command = new WebConfigurationQueryCommand(domainPath, WebConfigurationType.CMS_PLAYER);
			...
			if ((configs != null) && (configs.size() > 0)) {
				for (final Document config : configs) {
					final String documentType = config.getProperties().getString(WebConfigurationHelper.CODE);
					final String playerInstance = config.getProperties().getString(WebConfigurationHelper.ADDITIONAL_CODE);

					if (doc.getType().equals(documentType) && this.players.containsKey(playerInstance)) {

						final Map<String, String> windowProperties = new HashMap<String, String>();
						final PropertyList list = config.getProperties().getList(WebConfigurationHelper.OPTIONS);

						// Inject defined values in conf in the player properties map
						for (final Object o : list.list()) {
							if (o instanceof PropertyMap) {
								final PropertyMap map = (PropertyMap) o;
								windowProperties.put(map.get(WebConfigurationHelper.OPTION_KEY).toString(), map.get(WebConfigurationHelper.OPTION_VALUE).toString());
	...
	public String getDefaultExternalViewer(CMSServiceCtx ctx) {
		...
		final String externalUrl = NuxeoConnectionProperties.getPublicBaseUri().toString() + "/nxdoc/default/" + doc.getId() + "/view_documents";
	...
	public String createPortletDelegatedExternalLink(CMSServiceCtx ctx) {
		...
		final ResourceURL resourceURL = ctx.getResponse().createResourceURL();
	...
	public Link createCustomLink(CMSServiceCtx ctx) throws Exception {
		...
		if (!"detailedView".equals(ctx.getDisplayContext())) {
			...
			if (("File".equals(doc.getType()) || "Audio".equals(doc.getType()) || "Video".equals(doc.getType())) && ("download".equals(ctx.getDisplayContext()))) {
				final PropertyMap attachedFileProperties = doc.getProperties().getMap("file:content");
				if ((attachedFileProperties != null) && !attachedFileProperties.isEmpty()) {

					// Nuxeo controller
					final NuxeoController nuxeoCtl = new NuxeoController(ctx.getRequest(), ctx.getResponse(), ctx.getPortletCtx());
		...
		if (url != null) {
			final Link link = new Link(url, externalLink);
	...
	public void formatContentMenuBar(CMSServiceCtx ctx) throws Exception {
		...
		final List<MenubarItem> menuBar = (List<MenubarItem>) ctx.getRequest().getAttribute(Constants.PORTLET_ATTR_MENU_BAR);
	...
	public String addPublicationFilter(CMSServiceCtx ctx, String nuxeoRequest, String requestFilteringPolicy) throws Exception {
		...
		if (portalName != null) {
			final PortalObjectContainer portalObjectContainer = (PortalObjectContainer) invocation.getAttribute(Scope.REQUEST_SCOPE, "osivia.portalObjectContainer");
			final PortalObject po = portalObjectContainer.getObject(PortalObjectId.parse("", "/" + portalName, PortalObjectPath.CANONICAL_FORMAT));

			if (requestFilteringPolicy != null) {
				policyFilter = requestFilteringPolicy;
			} else {
				// Get portal policy filter
				final String sitePolicy = po.getProperty(InternalConstants.PORTAL_PROP_NAME_CMS_REQUEST_FILTERING_POLICY);
				if (sitePolicy != null) {
					...
					final String portalType = po.getProperty(InternalConstants.PORTAL_PROP_NAME_PORTAL_TYPE);
			...
			if (InternalConstants.PORTAL_CMS_REQUEST_FILTERING_POLICY_LOCAL.equals(policyFilter)) {
				...
				for (final PortalObject child : ((Portal) po).getChildren(PortalObject.PAGE_MASK)) {
					final String cmsPath = child.getDeclaredProperty("osivia.cms.basePath");
			...
			final String extraFilter = this.getExtraRequestFilter(ctx, requestFilteringPolicy);
			if (extraFilter != null) {
		...
		try {
			final Pattern ressourceExp = Pattern.compile("(.*)ORDER([ ]*)BY(.*)");

			final Matcher m = ressourceExp.matcher(editedNuxeoRequest.toUpperCase());
			m.matches();
	...
	public String getExtraRequestFilter(CMSServiceCtx ctx, String requestFilteringPolicy) throws Exception {
		...
		if (domainPath != null) {
			// get configs installed in nuxeo
			final WebConfigurationQueryCommand command = new WebConfigurationQueryCommand(domainPath, WebConfigurationType.EXTRA_REQUEST_FILTER);
			Documents configs = null;
			...
			if (configs.size() > 0) {
				...
				int i = 0;
				for (final Document config : configs) {
					final String nxqlCode = config.getProperties().getString(WebConfigurationHelper.CODE);
					if (nxqlCode != null) {
	...
	public String transformHTMLContent(CMSServiceCtx ctx, String htmlContent) throws Exception {
		...
		try {
			final Transformer transformer = WysiwygParser.getInstance().getTemplate().newTransformer();

			transformer.setParameter("bridge", new XSLFunctions(this, ctx));
			final OutputStream output = new ByteArrayOutputStream();
			final XMLReader parser = WysiwygParser.getInstance().getParser();
	...
	public String transformLink(CMSServiceCtx ctx, String link) {
		final XSLFunctions xslFunctions = new XSLFunctions(this, ctx);
	...
	public Link getLinkFromNuxeoURL(CMSServiceCtx cmsContext, String url) {
		...
		final PortalControllerContext portalControllerContext = new PortalControllerContext(cmsContext.getControllerContext());

		if (StringUtils.startsWith(url, "/nuxeo/")) {
			// Nuxeo URL
			final String nuxeoURL = StringUtils.substringBefore(url, "#");
			// Anchor
			final String anchor = StringUtils.substringAfter(url, "#");

			// CMS path
			final String cmsPath = this.transformNuxeoURL(cmsContext, nuxeoURL);

			String currentPagePath = null;
			if (cmsContext.getRequest() != null) {
				final Window window = (Window) cmsContext.getRequest().getAttribute("osivia.window");
				if (window != null) {
			...
			try {
				...
				final String serverName = cmsContext.getRequest().getServerName();
	...
	private String transformNuxeoURL(CMSServiceCtx cmsContext, String url) {
		...
		if (StringUtils.startsWith(url, "/nuxeo/web/")) {
			// Nuxeo web path
			final String webPath = StringUtils.removeStart(url, "/nuxeo/web/");

			final String[] splittedWebPath = StringUtils.split(webPath, "/");
			if (splittedWebPath.length > 1) {
				// Domain ID
				final String domainId = splittedWebPath[0];
				// Web ID
				final String webId = splittedWebPath[splittedWebPath.length - 1];
				// Service web ID
				final String serviceWebId = this.getWebIdService().domainAndIdToFetchInfoService(domainId, webId);

				// Document path
				String path = null;
				try {

					final CMSPublicationInfos pubInfos = this.getCmsService().getPublicationInfos(cmsContext, serviceWebId);
				...
				// CMS item
				final CMSItem cmsItem = new CMSItem(path, domainId, webId, null, null);
	...
	public Map<String, CMSItemType> getCMSItemTypes() {
		if (this.cmsItemTypes == null) {
			final List<CMSItemType> defaultTypes = this.getDefaultCMSItemTypes();
			this.cmsItemTypes = new LinkedHashMap<String, CMSItemType>(defaultTypes.size());
			for (final CMSItemType defaultType : defaultTypes) {
	...
	private List<CMSItemType> getDefaultCMSItemTypes() {
		final List<CMSItemType> defaultTypes = new ArrayList<CMSItemType>();

		// Workspace
		defaultTypes.add(new CMSItemType("Workspace", true, false, false, false, false, false, Arrays.asList("Folder", "DocumentUrlContainer", "AnnonceFolder", "PictureBook"), "/default/templates/workspace", "glyphicons glyphicons-wallet", true));
		// Portal site
		...
		// Folder
		defaultTypes.add(new CMSItemType("Folder", true, true, true, false, false, true, Arrays.asList("File", "Audio", "Video", "Folder", "Note", "Picture", "ContextualLink", "PictureBook", "ToutaticePad"), null, "glyphicons glyphicons-folder-closed"));
		// Ordered folder
		...
		// Annonce
		defaultTypes.add(new CMSItemType("Annonce", false, false, false, false, false, true, new ArrayList<String>(0), null, "glyphicons glyphicons-newspaper"));
		// Annonce folder
		...
		// Contextual link
		defaultTypes.add(new CMSItemType("ContextualLink", false, false, false, false, false, true, new ArrayList<String>(0), null, "glyphicons glyphicons-link"));
	...
	public String getCommentsHTMLContent(CMSServiceCtx cmsContext, Document document) throws CMSException {
		final List<CommentDTO> comments = this.getNuxeoCommentsService().getDocumentComments(cmsContext, document);
		final CommentsFormatter formatter = new CommentsFormatter(comments);
		final Locale locale = cmsContext.getRequest().getLocale();
	 ...
	 *
	 * @param navigationItemAdapter
	 *            the navigationItemAdapter to set
	 */
	...
	public String getContentWebIdPath(CMSServiceCtx cmsCtx) {
		final Document doc = (Document) cmsCtx.getDoc();

		final String webId = doc.getString("ttc:webid");
		final String domainId = doc.getString("ttc:domainID");
		...
		if (StringUtils.isNotEmpty(webId) && StringUtils.isNotEmpty(domainId)) {
			final String explicitUrl = doc.getString("ttc:explicitUrl");
			final String extension = doc.getString("ttc:extensionUrl");
			...
			final CMSItem cmsItem = new CMSItem(doc.getPath(), domainId, webId, properties, doc);
	...
	public String refreshUserAvatar(CMSServiceCtx cmsCtx, String username) {
		...
		final String avatarTime = Long.toString(new Date().getTime());
	...
	public BinaryDelegation validateBinaryDelegation(CMSServiceCtx cmsCtx, String path) {
		...
		final String id = cmsCtx.getServletRequest().getSession().getId();
		final Map<String, BinaryDelegation> delegationMap = delegations.get(id);
	...
	public Link getBinaryResourceURL(CMSServiceCtx cmsCtx, BinaryDescription binary) throws CMSException {
		...
		final BinaryDelegation delegation = new BinaryDelegation();

		try {
			String portalName = null;
			final ServerInvocation invocation = cmsCtx.getServerInvocation();
			...
			if (binary.getDocument() != null) {
				final Document doc = (Document) binary.getDocument();
			...
			final Map<String, BinaryDelegation> delegationMap = this.getUserDelegation(cmsCtx);
			delegationMap.put(path, delegation);

			final boolean refresh = PageProperties.getProperties().isRefreshingPage();

			// File name
			String fileName = binary.getFileName();
			if (fileName == null) {
				if (binary.getDocument() != null) {
					final Document document = (Document) binary.getDocument();
					final PropertyMap fileMap = document.getProperties().getMap("file:content");
					if (fileMap != null) {
						fileName = fileMap.getString("name");
					}
				} else if ((binary.getIndex() != null) && (cmsCtx.getDoc() != null)) {
					final int index = NumberUtils.toInt(binary.getIndex());
					final Document document = (Document) cmsCtx.getDoc();
					final PropertyList files = document.getProperties().getList("files:files");
					if ((files != null) && (files.size() > index)) {
						final PropertyMap fileMap = files.getMap(index);
						if (fileMap != null) {
	...
	public String refreshBinaryResource(CMSServiceCtx cmsCtx, String path) {
		...
		// renew the timestamp and map it to the user
		final String pathTime = Long.toString(new Date().getTime());
