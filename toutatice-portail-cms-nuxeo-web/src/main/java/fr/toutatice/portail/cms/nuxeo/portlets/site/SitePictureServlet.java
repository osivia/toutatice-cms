...
public class SitePictureServlet extends HttpServlet

{
	...
	public String formatResourceLastModified() {
		...
		SimpleDateFormat inputFormater = new SimpleDateFormat(
	...
	public boolean serveResourceByCache(HttpServletRequest resourceRequest,
			HttpServletResponse resourceResponse) throws PortletException,
			IOException {
	...
	public void doGet(HttpServletRequest theRequest,
		...
		try {
			...
			CMSBinaryContent content = (CMSBinaryContent) ctx
					.executeNuxeoCommand(new FileContentCommand(docPath,
							"file:content"));

			// Les headers doivent être positionnées avant la réponse
			theResponse.setContentType(content.getMimeType());

			theResponse.setHeader("Cache-Control", "max-age="
					+ PICTURE_TIMEOUT);

			theResponse
					.setHeader("Last-Modified", formatResourceLastModified());

			ResourceUtil.copy(new FileInputStream(content.getFile()),
					theResponse.getOutputStream(), 4096);
