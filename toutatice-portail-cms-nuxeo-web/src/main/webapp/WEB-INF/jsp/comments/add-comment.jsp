...
<div id="<%=commentId%>" class="add-comment">
	<form id="add-com-form" method="post" action="${valid}" class="ajax-form">	
		<textarea id="addedContent" name="content" rows="3" cols="60"></textarea><br/>
		<div id="errorAddCom" class="contentError"></div>
		<input type="submit" name="addComment"  value="Poster" class="ajax-link">
		<input type="reset" name="noAddComment"  value="Annuler" onclick="hideCommentField('div_add_comment<%= containerId %>');showCommentField('fake_add_comment<%= containerId %>');">
	</form>
</div>